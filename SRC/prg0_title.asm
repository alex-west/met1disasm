...
L80AF:  beq L80BE                       ;$6700 thru $673F = #$C0.
...
L811A:  BNE L812B                       ;so routine will keep running.
...
L814F:  BNE L8162                       ;last routine before continuing.
...
L8159:  BNE L815F                       ;go to next title routine, else continue-->
...
L8245:  BNE L8262                       ;to exit, else run this rouine.
...
L8265:  BNE L8282                       ;to exit, else run this rouine.
...
L82A5:  BNE L82E9                       ;to exit, else run this rouine.
...
L87B6:  DEX                             ;
L87B7:  BPL L87AD                       ;Loop until all values from table below are loaded.
L87B9:  LDA #$6B                        ;
...
L87E4:  BNE L8819                       ;If sprite is already done, skip routine.
...
L8800:  BNE L8816                       ;sprite graphic.
...
L882C:  LDA ($00),Y                     ;
L882E:  BPL L8835                       ;If data byte MSB is set, set $6EA9 to #$01 and move to-->
...
L8873:  BCC L887A                       ;and lower three bits are converted to twos-->
...
L88AA:
      + DEX                             ;
L88AB:  BPL L889D                       ;Loop until all data is loaded.
...
L8900:  BEQ L8936                       ;Has First4SlowCntr already hit 0? If so, branch.
L8902:  DEC First4SlowCntr              ;
L8904:  BNE L8936                       ;Is First4SlowCntr now equal to 0? if not, branch.
...
L8949:  BEQ L894F                       ;4 sprites has ended.  If so, start drawing those sprites.
...
L8966:  BNE L8975                       ;its movements, exit this routine.
L8968:  JSR UpdateSpriteCoords          ;($981E)Calculate new sprite position.
L896B:  BCS L8972                       ;If sprite not at final position, branch to move next frame.
...
L8978:  BEQ L89A9                       ;branch to exit.
L897A:  LDY CrossDataIndex              ;
L897C:  CPY #$04                        ;Check to see if at last index in table.  If so, branch-->
L897E:  BCC L8986                       ;to draw cross sprites.
L8980:  BNE L89A9                       ;If beyond last index, branch to exit.
L8982:  LDA #$00                        ;
...
L899C:  BNE L8991                       ;if not, branch to load next sprite data byte.
L899E:  CPY $00                         ;Has all the sprites been loaded for cross graphic?-->
L89A0:  BNE L898D                       ;If not, branch to load next set of sprite data.

L89A2:  LDA FrameCount                  ;
L89A4:  LSR                             ;Increment index into CrossSpriteIndexTbl every-->
L89A5:  BCC L89A9                       ;other frame.  This updates the cross sprites-->
...
L8AD0:  BEQ L8AD3                       ;Is any other PPU data waiting? If so, exit.
L8AD2:
      + RTS                             ;
...
L8B99:  BCC L8B82                       ;branch to process next unique item.
L8B9B:  RTS                             ;
...
L8BAB:  CMP $01                         ;
L8BAD:  BEQ UniqueItemFound             ;If unique item found, branch to UniqueItemFound.
L8BAF:
      + INX                             ;
L8BB0:  CPX #$3C                        ;If the unque item is a Zeebetite, return-->
L8BB2:  BCC L8B9E                       ;else branch to find next unique item.
L8BB4:  RTS                             ;
...
L8BEC:  LDX $06                         ;
L8BEE:  BEQ ProcessNewItemByte          ;If start of new byte, branch.
...
L8C64:  BCS L8C69                       ;Branch if incorrect password.
...
L8C8F:  BEQ L8C9E                       ;If so, restore mother brain, zeebetites and-->
...
L8CBF:  AND #$80                        ;
L8CC1:  BEQ L8CC9                       ;If statue not up, branch.
L8CC3:  LDA $00                         ;
...
L8CCC:  AND #$01                        ;
L8CCE:  BEQ L8CD6                       ;Branch if Kraid not yet defeated.
L8CD0:  LDA $00                         ;
...
L8CD9:  AND #$80                        ;
L8CDB:  BEQ L8CE3                       ;Branch if Ridley statue not up.
L8CDD:  LDA $00                         ;
...
L8CE3:
      + LDA RidleyStatueStatus          ;
L8CE6:  AND #$02                        ;
L8CE8:  BEQ L8CF0                       ;Branch if Ridley not yet defeated.
L8CEA:  LDA $00                         ;
...
L8D07:  BEQ L8D00                       ;When any of the 4 LSB are set. (Does not-->
...
L8D15:  BNE L8D3C                       ;further password processing.
...
L8D39:  DEY                             ;
L8D3A:  BPL L8D33                       ;Loop to load all 3 age bytes.
L8D3C:
      + RTS                             ;
...
L8DA5:  BNE IncrementToNextItem         ;#$24. If it matches, missiles have been found.
