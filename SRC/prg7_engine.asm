...
LC04A:  iny                             ;
LC04B:  bne LC048                       ;Repeat for entire page.
LC04D:  dec $01                         ;Decrement high byte of address.
LC04F:  bmi LC057                       ;If $01 < 0, all pages are cleared.
LC051:  ldx $01                         ;
...
LC0CB:  tay                             ;
LC0CC:  lda NMIStatus                   ;
LC0CE:  bne LC0D3                       ;If nonzero, NMI has ended. Else keep waiting.
...
LC0E9:  lda NMIStatus                   ;
LC0EB:  bne LC103                       ;Skip if the frame couldn't finish in time.
LC0ED:  lda GameMode                    ;
LC0EF:  beq LC0F4                       ;Branch if mode=Play.
...
LC116:  beq LC11B                       ;Branch if mode=Play.
...
LC11F:  beq LC13c                       ;if not, execute current routine as normal.

LC121:  lda MainRoutine                 ;
LC123:  cmp #$03                        ;Is game engine running?-->
LC125:  beq LC12F                       ;If yes, check for routine #5 (pause game).
LC127:  cmp #$05                        ;Is game paused?-->
LC129:  bne LC13c                       ;If not routine #5 either, don't care about START being pressed.
LC12B:  lda #$03                        ;Otherwise, switch to routine #3 (game engine).
LC12D:  bne LC131                       ;Branch always.
LC12F:
...
LC195:
      + sta PPUIOReg                    ;
LC198:  dey                             ;
LC199:  bne LC195                       ;Loops until the desired name table is cleared.-->
...
LC1B1:  iny                             ; 
LC1B2:  bne LC1AF                       ;Loop while more sprite RAM to clear.
LC1B4:  lda GameMode                    ;
...
LC1DA:  inx                             ;
LC1DB:  cpx #$E0                        ;
LC1DD:  bcc LC1D8                       ;Loop until all desired addresses are cleared.
LC1DF:  rts                             ;
...
CheckPalWrite:
LC1E0:  lda GameMode                    ;
LC1E2:  beq LC1ED                       ;Is game being played? If so, branch to exit.
...
LC1ED:
      + ldy PalDataPending              ;
LC1EF:  bne LC1FF                       ;Is palette data pending? If so, branch.
LC1F1:  lda GameMode                    ;
LC1F3:  beq LC1FE                       ;Is game being played? If so, branch to exit.
...
LC237:  bne LC22A                       ;status of all 8 buttons.
...
LC241:  eor $00                         ;
LC243:  beq LC24D                       ;Branch if no buttons changed.
LC245:  lda $00                         ;                       
...
LC259:  bne LC263                       ;If none, branch.
...
DecTimer:
LC272:  lda Timer1,x                    ;
LC274:  beq LC278                       ;Don't decrease if timer is already zero.
LC276:  dec Timer1,x                    ;
LC278:
...
LC2AC:  sta $00                         ;
LC2AE:  bcc LC2B2                       ;Increment $01(upper address byte) if carry-->
...
LC2B7:  sta $02                         ;
LC2B9:  bcc LC2BD                       ;Increment $01(upper address byte) if carry-->
...
LC2CC:  beq LC2E3                       ;If zero no PPU data to write, branch to exit.
LC2CE:  lda #$A1                        ;                       
...
LC354:  dec $06                         ;
LC356:  bne LC34E                       ;Branch if more horizontal tiles to replace.
...
LC364:  dec $04                         ;
LC366:  bne LC33D                       ;Branch if more lines need to be changed on name table.
...
LC3BF:  lda ($02),y                     ;
LC3C1:  bne LC385                       ;Is PPU string done loading (#$00)? If so exit,-->
...
LC3F8:  bcs LC3F2                       ;Branch to add #$5F to create valid result.
LC3FA:  rts                             ;
...
LC42F:
      + lda NMIStatus                   ;
LC431:  beq LC42F                       ;Wait for NMI to end.
LC433:  rts                             ;
...
LC442:
      + lda NMIStatus                   ;
LC444:  beq LC442                       ;Wait for NMI to end before continuing.
LC446:  rts                             ;
...
LC479:  ora #$1E                        ;
LC47B:  bne WriteAndWait                ;Branch always.
...
LC7F0:  lda $06                         ;
LC7F2:  beq LC800                       ;If copying complete, branch to exit.
...
LC83D:
      + inx                             ;
LC83E:  bne LC830                       ;Loop until all required RAM is cleared.
...
LC964:  bne LC92A                       ;exit if not.
LC966:  lda AnimDelay                   ;Is Samus still exploding?-->
LC969:  bne LC92A                       ;Exit if still exploding.
...
LC9A3:  beq LC99B                       ;Branch if middle age overflowed, need to increment--> 
LC9A5:
...
LCA50:  beq LoadGameData                ;to load Samus' saved game info.
...
LCA5E:  dey                             ;
LCA5F:  bpl LCA59                       ;Loop until unique item history transfer complete.
...
LCA6D:  inx                             ;
LCA6E:  cpx #$10                        ;
LCA70:  bne LCA66                       ;Loop until Samus' data transfer complete.
...
LCA8F:  inx                             ;
LCA90:  cpx #$10                        ;
LCA92:  bne LCA88                       ;Loop until Samus' data transfer complete.
LCA94:  pla                             ;
LCA95:  rts                             ;
...
LCAB3:
      + iny                             ;
LCAB4:  bne LCAAB                       ;Lop until all saved game data is erased.
...
LCAC0:  inx                             ;
LCAC1:  cpx #$0C                        ;
LCAC3:  bne LCABC                       ;Loop until all data is erased.
LCAC5:  rts                             ;
...
LCADE:  beq LCAEE                       ;Exit if at saved game 0.  No further calculations required.
LCAE0:
      + lda $00                         ;
LCAE2:  clc                             ;
LCAE3:  adc #$40                        ;
...
LCDA1:  bne LCDBE                       ;If not, branch to exit.
LCDA3:  lda SamusGear                   ;
LCDA6:  and #gr_SCREWATTACK             ;Does Samus have screw attack?-->
LCDA8:  beq LCDBE                       ;If not, branch to exit.
...
LCEE2:  beq +                           ;
LCEE4:  bcs LCF2B                       ;Samus not dead. Branch to exit.
...
LDB48:  beq LDB36                           ;If so, branch to exit.
...
LDB67:  bcc LDB36                       ;If so, branch to exit.
...
LDBCC:  bcs LDBF7                       ;If so, branch.
LDBCE:  cpy #$06                        ;Is item the wave beam or ice beam?-->
LDBD0:  bcc LDBDA                       ;If not, branch.
...
LDC28:  beq LDC34                       ;Branch if scrolling up/down.
LDC2A:  bcc LDC3C                       ;Branch if scrolling right.

;Scrolling left.
LDC2C:  lda ScrollX                     ;Unless the scroll x offset is 0, the actual room x pos-->
LDC2E:  beq LDC3C                       ;needs to be decremented in order to be correct.
LDC30:  dec $07                         ;
LDC32:  bcs LDC3C                       ;Branch always.
...
LDC43:  clc                             ;
LDC44:  beq LDC4D                       ;If Scrolling up/down, branch to adjust item y position.
...
LDCE8:  bpl LDCEF                       ;If MSB is set in ObjectCntrl, use its flip bits(6 and 7).
...
LDCF5:  jsr ClearObjectCntrl            ;($DF2D)Clear object control byte.
        ...
        pla
        ldx PageIndex
LDCFC:  lda InArea ; Branch ahead if not in Tourian
        cmp #$13
        bne Lx135
...
LDE40:  bcs LDE44                       ;If number is still a positive number, branch to store value.
...
LDEDB:  tax                             ;
LDEDC:  beq LDEE3                           ;Branch if object is not within the screen boundaries.
...
LDF83:  ldy IsSamus                     ;
LDF85:  bne LDF8F            ;Is Samus the object exploding? if so, branch.
...
LDF8C:  jmp LDF91                          ;Jump to load explode placement data.
...
LDFA0:  jmp LDF75                       ;Branch to add y placement values to sprite coords.
...
LDFAD:
      + bit $04                         ;
LDFAF:  bvc LDFB6                           ;Branch if bit 6 cleared, else data is negative displacement.
...
LDFC9:  lsr                             ;
LDFCA:  bcs LDFD2                       ;Check if LSB is set. If not, the byte stored on stack-->
...
LDFDC:  jmp LDFAD                       ;Branch to add x placement values to sprite coords.
...
LDFFE:  beq LE012                           ;
LE000:  bcs LE01A                          ;If carry is still set, sprite is not in screen boundaries.
LE002:  lda $10                         ;
...
LE018:  bcc LE01B                          ;boundaries.  If so, branch.
LE01A:
...
LE022:  beq LE02E                           ;If not, branch.
LE024:  bcs LE036                          ;If carry is still set, sprite is not in screen boundaries.
LE026:  lda $09                         ;
LE028:  clc                             ;If a portion of the object is outside the sceen-->
LE029:  adc $0E                         ;boundaries, treat object as if the whole thing is-->
LE02B:  bcc LE037                         ;not visible.
...
LE034:  bcc LE037                          ;boundaries.  If so, branch.
LE036:
      + dex                             ;Sprite is not within screen boundaries. Decrement X.
LE037:
      + rts                             ;
...
LE09B:  beq LE0BB                         ;If so, branch to exit.
LE09D:  ldy EnAnimDelay,x               ;
LE0A0:  beq LE0A7                           ;Check if current anumation frame is ready to be updated.
LE0A2:  dec EnAnimDelay,x               ;Not ready to update. decrement delay timer and-->
LE0A5:  bne LE0BB                         ;branch to exit.
LE0A7:
...
LE0B1:  beq LE0BC                          ;If so, branch to reset animation.
...
LE0D1:  bne LE0C7                           ;load next byte from table.
...
LE0EC:  bne LE11C                          ;If so, branch.
LE0EE:  ldy MaxMissiles                 ;
LE0F1:  beq LE10A                           ;Don't show missile count if Samus has no missile containers.
...
LE14C:  lda TankCount                   ;
LE14F:  beq LE172                          ;Branch to exit if Samus has no energy tanks.
...
LE169:  bne LE16C                           ;If so, then branch.-->
...
LE1B3:  bcs LE1B0                           ;is not set.  The resulting number of loops is the decimal-->
...
LE1E6:  bcs LE1ED                           ;If the shifted bit was 1, Branch out of loop.
LE1E8:  inx                             ;Increment X to keep of # of bits checked.
LE1E9:  cpx #$08                        ;Have all 8 bit been tested?-->
LE1EB:  bne LE1E5                           ;If not, branch to check the next bit.
LE1ED:
...
LE1F3:  beq LE1F0                           ;Exit if Samus isn't in a door.
LE1F5:  dex                             ;
LE1F6:  bne LE1FE                           ;Not in right door. branch to check left door.
LE1F8:  jsr ScrollRight                 ;($E6D2)DoorStatus=1, scroll 1 pixel right.
LE1FB:  jmp LE204                          ;Jump to check if door scroll is finished.
...
LE20C:
      + dex                             ;
LE20D:  bne LE215                           ;Check if need to scroll down to center door.
LE20F:  jsr ScrollDown                  ;($E519)DoorStatus=3, scroll 1 pixel down.
LE212:  jmp VerticalRoomCentered                          ;Jump to check y scrolling value.
LE215:
      + dex                             ;
...
LE227:  bmi LE241                          ;If so, branch.
LE229:  inx                             ;X=2. Samus is in left door.
LE22A:  bne LE241                          ;Branch always.
...
LE235:  bcs LE23D                           ;bridge room, etc.)? If so, branch to NOT toggle scrolling.
...
LE23B:  bcc LE241                          ;by the routine just above.
LE23D:
      + lda #$47                        ;Set mirroring for vertical mirroring(horz scrolling).
LE23F:  bne LE244                          ;Branch always.
...
LE292:  bne LE2A4                          ;but only once every 8 frames if she does.
LE294:  lda SamusGear                   ;
LE297:  and #gr_VARIA                   ;Does Samus have the Varia?-->
LE299:  beq LE29D                           ;If not, branch.
LE29B:  bcc LE2A4                          ;Samus has varia. Carry set every other frame. Half damage.
LE29D:
      + lda #$07                        ;
...
LE2C7:  bcs LE2D3                           ;Branch if Samus successfully moved up 1 pixel.
...
LE2D5:  bne LE2C4                          ;Branch if Samus needs to be moved another pixel.
...
LE2E6:  bcs LE316                         ;Branch if Samus successfully moved down 1 pixel.

;Samus bounce after hitting the ground in ball form.
LE2E8:  lda ObjAction                   ;
LE2EB:  cmp #sa_Roll                    ;Is Samus rolled into a ball?-->
LE2ED:  bne LE30B                           ;If not, branch.
LE2EF:  lsr ObjVertSpeed                ;Divide verticle speed by 2.
LE2F2:  beq LE30E                          ;Speed not fast enough to bounce. branch to skip.
...
LE35A:  bne LE352                           ;Branch if Samus needs to be moved another pixel.
...
LE3B9:  bpl LE3C9                           ;Branch if Samus is moving downwards.
...
LE460:  bne Lx150                       ;Only call crash detection every 8th pixel.
...
LE61C:  dec $04                         ;
LE61E:  bne LE616                           ;Loop until all attrib data loaded into PPU.
...
LE72D:  and $006C,y                     ;
LE730:  sec                             ;
LE731:  bne LE76F                       ;Can't load room, a door is in the way. This has the-->
...
LE754:  beq LE76F                        ;If so, branch to exit with carry flag set.

LE756:  sta RoomNumber                  ;Store room number.

LE758:
      + cmp $95D0,y                     ;Is it a special room?-->
LE75B:  beq LE76A                           ;If so, branch to set flag to play item room music.
LE75D:  iny                             ;
LE75E:  cpy #$07                        ;
LE760:  bne LE758                           ;Loop until all special room numbers are checked.

LE762:  lda ItemRoomMusicStatus         ;Load item room music status.
LE764:  beq LE76C                          ;Branch if not in special room.
LE766:  lda #$80                        ;Ptop playing item room music after next music start.
LE768:  bne LE76C                          ;Branch always.
...
LEA2D:  cmp #$FF                        ;
LEA2F:  beq LEA2A                           ;Branch to exit if room is undefined.
LEA31:  cmp #$FE                        ;
LEA33:  beq LEA5D                           ;Branch if empty place holder byte found in room data.
LEA35:  cmp #$F0                        ;
LEA37:  bcs LEA13                          ;Branch if time to write PPU attribute table data.
...
LEA79:  bcc LEA7D                           ;If carry occurred, increment high byte of pointer-->
LEA7B:  inc CartRAMWorkPtr+1            ;in room RAM.
LEA7D:
      + dex                             ;
LEA7E:  bne LEA72                          ;Repeat until at desired y position(X=0).
...
LEAC1:  adc RoomPtr                     ;
LEAC3:  sta RoomPtr                     ;
LEAC5:  bcc LEAC9                           ;Did carry occur? If not branch to exit.
...
LEE3B:  bne LEDFB                         ;Branch always to exit handler routines.
...
LEE4D:  beq LEE61                         ;Samus has no unique items. Load item and exit.
LEE4F:
      + lda $07                         ;
...
LEE5B:  beq LEE62                         ;Samus already has item. Branch to exit.
LEE5D:
      + dey                             ;
LEE5E:  dey                             ;
LEE5F:  bne LEE4F                          ;Loop until all Samus' unique items are checked.
LEE61:
...
LEF2F:  beq LEF38                           ;to see if the attribute table data for the room RAM has-->
LEF31:  cmp #$67                        ;been reached.  If so, branch to check lower byte as well.
LEF33:  bcc LEF3F                          ;If not at end of room RAM, branch to draw macro.
LEF35:  beq LEF38                           ;
...
LEF3C:  bcc LEF3F                           ;If not, branch to draw the macro.
...
LEF57:  bpl LEF4B                           ;If not, loop to do another.
...
LEF65:  bne LEF72                           ;If yes, branch to do another macro.
...
LEF94:  beq LEF99                           ;If so, branch to exit.
...
LEFA2:  beq LEFF3                       ;If so, no need to modify the attribute table, exit.
...
LEFED:  bcc LEFE8                           ;Loop until attribute table bits are in the proper location.
LEFEF:
      + ora ($02),y                     ;
...
LF014:  iny                             ;
LF015:  bne LF012                           ;Loop until entire attribute table is filled.
LF017:  rts                             ;
...
LF270:  ora SamusHit,x
        sta SamusHit,x
        rts

LF277:  bcs Exit17
LF279:  lda $10
LF27B:  ora SamusHit,y
        sta SamusHit,y
        Exit17:
        rts
...
LFFBA:
      + LDA PPUStatus                   ;
LFFBD:  BPL LFFBA                           ;Wait for VBlank
LFFBF:
      + LDA PPUStatus                   ;
LFFC2:  BPL LFFBF                           ;
LFFC4:  ORA #$FF                        ;
